...
describe 'class solver' do
  ...
  context 'tests for factorial' do
    ...
    it 'factorial return 1 for 0' do
      expect(@solver.factorial(0)).to eql 1
    end

    it 'raises an exeption for negative arguments' do
      expect { @solver.factorial(-2) }.to raise_error('num has to be >= 0')
    end
    ...
    it 'raises an exception for string value' do
      expect { @solver.factorial('ab') }.to raise_error('Only numbers are accepted')
    end
  end

  context 'tests for reverse method' do
    it 'raise exception for non string values' do
      expect { @solver.reverse(333) }.to raise_error('Only String values are accepted')
    end
  ...
  context 'tests for fizzbuzz methode' do
    it 'raise an exception for non integer values' do
      expect { @solver.fizzbuzz('55') }.to raise_error('Only numbers are accepted')
    end
